...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/stego/metadata/service/{service}', function (Service $service) {
        abort_unless(auth()->user()?->isAdmin(), 403);

        return view('stego.metadata-service', [
            'service' => $service->loadMissing('galleryItem'),
        ]);
    })->name('stego.metadata.service');

    Route::get('/stego/metadata/closed-date/{closedDate}', function (ClosedDate $closedDate) {
        abort_unless(auth()->user()?->isAdmin(), 403);

        return view('stego.metadata-closed-date', [
            'closedDate' => $closedDate,
        ]);
    })->name('stego.metadata.closed-date');
