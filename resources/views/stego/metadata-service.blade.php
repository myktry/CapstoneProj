@php
    ...
    $pngBase64 = (string) ($service->metadata_stego_png_base64 ?? '');
...
<head>
    ...
    <title>Stego Metadata — Service #{{ $service->id }}</title>
...
<div class="mx-auto max-w-4xl space-y-8 px-4 py-10 sm:px-6 lg:px-8">
    <div>
        ...
        <h1 class="mt-2 text-2xl font-semibold text-white">Service #{{ $service->id }} metadata</h1>
        <p class="mt-1 text-sm text-zinc-400">
            Decodes stored `metadata_stego_png_base64` using stegano-kit AES-256-GCM and <code class="text-zinc-300">VITE_STEGO_SECRET</code>.
        </p>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-2">
        <div class="space-y-4 rounded-2xl border border-white/10 bg-zinc-900/60 p-5">
            ...
            @else
                <img id="stegoImg" src="{{ $dataUrl }}" alt="" class="w-full rounded-xl border border-white/10 bg-black"/>
            @endif
        </div>
        <div class="space-y-4 rounded-2xl border border-white/10 bg-zinc-900/60 p-5">
            <h2 class="text-sm font-semibold text-white">Decoded JSON</h2>
            <p id="decodeStatus" class="text-sm text-zinc-400">Waiting…</p>
            <pre id="decodedJson" class="whitespace-pre-wrap break-words rounded-xl border border-white/10 bg-zinc-950 p-4 text-xs text-zinc-200"></pre>
        </div>
    </div>
</div>
